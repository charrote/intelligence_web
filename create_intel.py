import sys, os
sys.path.insert(0, '/Users/Yoo/SVN/00.GITHUB/Intelligence_Web')
from core import db
...
for item in intel_data:
    intel_id = db.create_intelligence(
        db_path=research_db,
        title=item['title'],
    ...
    print(f'创建成功: ID={intel_id} | {item["title"][:40]}')

total = len(db.get_intelligences(research_db, {'limit': 100}))
print(f'\n总计: Research域现有 {total} 条情报')
